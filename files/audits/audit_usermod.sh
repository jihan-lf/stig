...
&&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/)" )"
  ...
  expected_on_disk="-a always,exit -F path=/usr/sbin/usermod -F perm=x -F auid>=${UID_MIN} -F auid!=unset -k usermod"
  expected_running="-a always,exit -S all -F path=/usr/sbin/usermod -F perm=x -F auid>=${UID_MIN} -F auid!=-1 -F key=usermod"
