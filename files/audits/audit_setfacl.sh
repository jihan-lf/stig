...
&&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/)" )"
  ...
  expected_on_disk="-a always,exit -F path=/usr/bin/setfacl -F perm=x -F auid>=${UID_MIN} -F auid!=unset -k perm_chng"
  expected_running="-a always,exit -S all -F path=/usr/bin/setfacl -F perm=x -F auid>=${UID_MIN} -F auid!=-1 -F key=perm_chng"
